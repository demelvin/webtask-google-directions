...
const DEFAULT_API_MODE = 'driving';
...
* @param {Boolean=} includeKey - true if the api key should be included
* @returns {String} requestUrl - the request url
*/
var getRequestUrl = (path, start, end, includeKey) => {
	const query = {
		origin: start, 
		destination: end,
...
*/
var fetchDirections = (username, start, end, cb) => {
	var options = {
		uri: getRequestUrl(GOOGLE_DIRECTIONS_API_URL, start, end, true),
		json: true
	};
	var message = {};
	rp(options)
...
module.exports = (ctx, cb) => {
	...
	//setup globals
	googleApiKey = ctx.secrets.GOOGLE_API_KEY;
	...
	if(!params){
		...
		//get the directions
		fetchDirections(username, params.start, params.end, (message) => {
